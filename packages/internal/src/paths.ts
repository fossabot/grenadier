...
const CONFIG_FILE_NAME = "grenadier.toml";
...
/**
 * Search the parent directories for the Grenadier configuration file.
 */
export const getConfigPath = (): string => {
	const configPath = findUp(CONFIG_FILE_NAME);
	if (!configPath) {
		throw new Error(
			`Could not find a "${CONFIG_FILE_NAME}" file, are you sure you're in a Grenadier project?`
		);
	}
	return configPath;
};
...
/**
 * The Grenadier config file is used as an anchor for the base directory of a project.
 */
...
/**
 * Path constants that are relevant to a Grenadier project.
 */
	...
	entries.forEach((entry) => {
		if (entry.isDirectory()) {
			...
			if (fs.existsSync(testFile)) {
				...
				deps.push(
					...processPagesDir(
						path.join(webPagesDir, entry.name),
						newPrefix
					)
				);
			}
