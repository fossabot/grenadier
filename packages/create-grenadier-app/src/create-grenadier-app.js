...
// This downloads the latest release of Grenadier from https://github.com/grenadierjs/create-grenadier-app/
// and extracts it into the supplied directory.
//
// Usage:
// `$ npx create-grenadier-app ./path/to/new-project`
...
const RELEASE_URL =
	"https://api.github.com/repos/grenadierjs/create-grenadier-app/releases/latest";
...
if (!targetDir) {
	...
	console.log(
		`  ${chalk.cyan("npx create-grenadier-app")} ${chalk.green(
			"<project-directory>"
		)}`
	);
	console.log();
...
const installNodeModulesTasks = ({ newAppDir }) => {
	return [
		{
			title: "Checking node compatibility",
			task: () => {
				return new Promise((resolve, reject) => {
...
new Listr(
	...
	.then(() => {
		...
		console.log();
		console.log(
			`Thanks for trying out Grenadier! We've created your app in '${newAppDir}'`
		);
		console.log();
		console.log(
			"Inside that directory you can run `npx gr dev` to start the development server."
		);
	})
	.catch((e) => {
		console.log();
